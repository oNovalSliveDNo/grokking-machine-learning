...
import pandas as pd
from .metrics import accuracy, gini_index, entropy, mae, mse
...
def evaluate_thresholds(
    ...
    for threshold in thresholds:
        ...
        # Если задача регрессии — считаем MSE и MAE
            ...
            weighted_mse = (len(left) * mse(left) + len(right) * mse(right)) / total_len
            weighted_mae = (len(left) * mae(left) + len(right) * mae(right)) / total_len
